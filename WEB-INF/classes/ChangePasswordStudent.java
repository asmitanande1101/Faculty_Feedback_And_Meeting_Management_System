...
public class ChangePasswordStudent extends HttpServlet
{
	public void doPost(HttpServletRequest req,HttpServletResponse res)
	{
		try
		{
			...
			String q="update Student set sPassword='"+nPass+"'where sPassword='"+cPass+"'";
			if(pass.equals(cPass))
			{
				if(nPass.equals(rPass))
				{
					st.execute(q);
					RequestDispatcher rd=req.getRequestDispatcher("StudentPasswordChanged.html");
					rd.forward(req,res);	
				}
				else
				{
					RequestDispatcher rd=req.getRequestDispatcher("StudentWrongPasswordCP.html");
					rd.forward(req,res);
				}	
			}
			else
